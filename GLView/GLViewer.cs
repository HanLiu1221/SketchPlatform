...
namespace SketchPlatform
{
    public class GLViewer : SimpleOpenGlControl
    {
        ...
        private int extractPageNum(string page)
        {
            ...
            string substr = page.Substring(0, _idx);
            if (substr[substr.Length - 1] >= 'a' && substr[substr.Length - 1] <= 'z')
            {
                substr = substr.Substring(0, substr.Length - 1);
            }
            int num = Int32.Parse(substr);
            return num;
        }
        ...
        public void sendDataToInsetViewer()
        {
            ...
            {
                int nextPage = this.extractPageNum(this.pageNumber[n]);
                if (nextPage > this.currPageIdx)
                {
                    --n;
                    break;
                }
            ...
            if (n < this.nSequence)
            {
                ...
                if (guideLinesIds.Count > 0)
                {
                    ...
                    cloneBox.highlightFaceIndex = highlightFaceIndex;
        ...
        private void resetAllBoxes()
        {
            foreach (Segment seg in this.currSegmentClass.segments)
            {
                ...
                this.setStrokeSize(SegmentClass.StrokeSize, seg, SegmentClass.StrokeColor, GuideLineColor);
            }
            this.activeSegment = null;
            this.currBoxIdx = -1;
        }
        ...
        private void activateDrawnBoxes()
        {
            ...
            foreach (Segment seg in this.currSegmentClass.segments)
            {
                ...
                this.setStrokeSize(SegmentClass.StrokeSize / 2, seg, SegmentClass.HiddenColor, GuideLineColor);
                foreach (GuideLine line in seg.boundingbox.edges)
                {
                    line.active = true;
                    line.strokeGap *= 0.6;
        ...
        private string getLineType(int i)
        {
            string s = "";
            switch(i)
            {
        ...
        public void captureScreenCropped(int idx)
        {
            double w = 297, h = 210;
            int wid = 850;
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            {
                if (this.drawStrokes.Count > 0)
                {
                    this.drawStrokes.RemoveAt(this.drawStrokes.Count - 1);
                    this.Refresh();
                    return;
                }
            }
        ...
        private void EraserMouseDown(Vector2d p, MouseEventArgs e)
        {
            ...
            if(e.Button == System.Windows.Forms.MouseButtons.Left){
                ...
                {
                    this.drawStrokes.Remove(stroke);
                }
            }
        }
                ...
                if (this.showMesh)
                {
                    if (this.drawFace)
                    {
                        if (this.isShowContour() || this.showDrawnStroke)
                        {
                            ...
                            this.drawMeshFace(seg.mesh, ModelColor, false);
                        }
                        else
                        {
